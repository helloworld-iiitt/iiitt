...
const Video = (props) => {
  ...
  return (
    <section className="videoSection" id={id}>
      <h4>
        Day {dayId+1}: {day.date} { session == "fn" ? "Morning" : "Afternoon" } session - Video {vidId+1}: {vid.title}
      </h4>
      <center>
        <Iframe src={vid.src} width="640" height="480" frameBorder="0"/>
        <a href={vid.src} target="_blank" className={classes.link}>Click here to open in a new tab in case the video player doesn't work</a>
      </center>
    </section>
  )
}
...
export default function VlaVideos(props) {
  ...
  useEffect(() => {
    import(`../../json/${path}.json`)
      ...
      .catch(() => {console.clear();console.log("HELLO");setRedirect(true)})
  }, [])
                    ...
                    <section className="list">
                      {
                        videos.map(( day, dayId ) => {
                          return (
                            <div className="day">
                              <strong>Day {dayId+1}: {day.date}</strong>
                                ...
                                <ul className="videoList">
                                  {
                                    day.fn.map(( vid, vidId) => {
                                      ...
                                      return (
                                        <li key={id}><a href={id} className={classes.link}>Video {vidId+1}: {vid.title}</a></li>
                                      )
                                    })
                                  }
                                </ul>
                    ...
                    <section className="videos">
                      {
                        videos.map(( day, dayId ) => {
                          return (
                            ...
                            {
                              day.fn.map((vid, vidId) => {
                                return <Video vid={vid} vidId={vidId} day={day} dayId={dayId} session="fn"/>
                              })
                            }

                            {
                              day.an.map((vid, vidId) => {
                                return <Video vid={vid} vidId={vidId} day={day} dayId={dayId} session="an"/>
                              })
                            }
