...
const Home: React.FC = () => {
  ...
  const resetTwitterTabCycle = (startFrom: number) => {
    if (twitterTabIntervalRef.current)
      clearInterval(twitterTabIntervalRef.current);
    ...
    (items?: Item[]) =>
      items
        ?.map((item) => {
          ...
          const diffDays =
            (today.getTime() - postedDate.getTime()) / (1000 * 3600 * 24);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [achRes, newsRes, eventsRes, noticeRes, carouselRes] =
          await Promise.all([
            ...
            fetch("/json/carousel/home_carousel.json").then((res) =>
              res.json(),
            ),
      ...
      <div className="container">
        ...
        <div className="row">
          ...
          <Paper elevation={3} className="tabbedPane" id="twitter_timeline">
            <Tabs
              value={twitterTab}
              onChange={handleTwitterTabChange}
              aria-label="Placements-SocialMedia"
            >
